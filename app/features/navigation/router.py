...
@router.callback_query(StateFilter("*"), F.data == CallbackData.BACK_MAIN)
async def back_main_handler(callback_query: CallbackQuery, state: FSMContext):
    await state.clear()
...
@router.callback_query(StateFilter("*"), F.data == CallbackData.BACK_ABOUT)
async def back_about_handler(callback_query: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback_query.answer()
