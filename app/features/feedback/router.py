...
from app.shared.keyboards import get_back_to_main_keyboard, get_feedback_keyboard
...
@router.message(Command(CommandsData.FEEDBACK), StateFilter("*"))
@catch_errors()
async def feedback_command(message: types.Message, state: FSMContext):
    await state.clear()
    await feedback_base_handler(message=message, state=state)


@router.message(F.text == TextCommandsData.FEEDBACK, StateFilter("*"))
@catch_errors()
...
async def handle_feedback(message: types.Message, state: FSMContext):
    ...
    if not security_validator.rate_limit_check(user_id, "feedback"):
        await message.answer(
            MessagesData.ERROR_FEEDBACK_LIMIT,
